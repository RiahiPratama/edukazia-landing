...
// Green palette — Islamic/growth association
const L = {bg:"#FDFCFA",bgW:"#EDFDF5",bgC:"#F0EDFF",card:"#FFFFFF",cb:"rgba(0,0,0,0.05)",cs:"0 2px 20px rgba(0,0,0,0.04)",ch:"0 12px 40px rgba(16,185,129,0.1)",tx:"#1A1535",sub:"#5E5878",mu:"#9D97B5",p:"#10B981",pS:"#ECFDF5",bl:"#3B82F6",blS:"#EFF6FF",gold:"#E6B800",goldS:"#FFF9E0",goldT:"#1A1535",pk:"#EC4899",pkS:"#FDF2F8",or:"#F59E0B",orS:"#FFFBEB",v:"#5C4FE5",vS:"#EEEDFC",coral:"#EF4444",nav:"rgba(253,252,250,0.92)",navB:"rgba(0,0,0,0.05)",footer:"#1A1535",cta:"linear-gradient(135deg,#10B981,#059669)",m:"light"};
const D = {bg:"#080E0B",bgW:"#0A120E",bgC:"#0B0A14",card:"rgba(255,255,255,0.03)",cb:"rgba(255,255,255,0.07)",cs:"0 2px 24px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)",ch:"0 16px 48px rgba(16,185,129,0.12), 0 0 0 1px rgba(255,255,255,0.08)",tx:"#F5F4FF",sub:"#A8A4BE",mu:"#5D596F",p:"#4ADE9A",pS:"rgba(74,222,154,0.1)",bl:"#70AEFF",blS:"rgba(112,174,255,0.08)",gold:"#F0C830",goldS:"rgba(240,200,48,0.08)",goldT:"#080E0B",pk:"#F78DC2",pkS:"rgba(247,141,194,0.08)",or:"#FFB84D",orS:"rgba(255,184,77,0.08)",v:"#8B7FF5",vS:"rgba(139,127,245,0.1)",coral:"#FF8A8A",nav:"rgba(8,14,11,0.85)",navB:"rgba(255,255,255,0.06)",footer:"#050805",cta:"linear-gradient(135deg,#10B981,#059669)",m:"dark"};
const fh="'Sora',sans-serif",fb="'Plus Jakarta Sans',sans-serif";
const WA="https://wa.me/6281384253679?text=Halo+EduKazia%2C+saya+tertarik+kursus+Bahasa+Arab";
...
function Nav({isDark,toggle}){
  ...
  return <>
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,background:sc?t.nav:"rgba(8,14,11,0.6)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${sc?t.navB:"rgba(255,255,255,0.04)"}`,transition:"all 0.35s"}}>
...
// ═══════ 1. HERO — Dark green, Arabic script visual ═══════
function Hero(){
  const t=useT();
  const aspects=["التجويد","المخارج","المحادثة","القراءة"];
  const labels=["Tajwid","Makhraj","Muhadatsah","Qira'ah"];
  const[ai,setAi]=useState(0);
  useEffect(()=>{const i=setInterval(()=>setAi(p=>(p+1)%aspects.length),2500);return()=>clearInterval(i);},[]);

  return <section style={{background:"linear-gradient(160deg,#0A1510 0%,#122820 40%,#0A1510 100%)",paddingTop:130,paddingBottom:80,position:"relative",overflow:"hidden"}}>
    <div style={{position:"absolute",top:"15%",left:"8%",width:280,height:280,borderRadius:"50%",background:"#10B981",opacity:0.04,filter:"blur(100px)"}}/>
    <div style={{position:"absolute",bottom:"10%",right:"12%",width:220,height:220,borderRadius:"50%",background:"#4ADE9A",opacity:0.03,filter:"blur(80px)"}}/>
    <W>
      <div className="hero-flex" style={{display:"flex",alignItems:"center",gap:52}}>
        <div style={{flex:"1 1 500px"}}>
          <Rv><div style={{fontFamily:fb,fontSize:13,fontWeight:700,color:"#4ADE9A",background:"rgba(74,222,154,0.08)",padding:"6px 18px",borderRadius:100,display:"inline-flex",alignItems:"center",gap:8,marginBottom:24,border:"1px solid rgba(74,222,154,0.15)"}}>
            <span style={{fontSize:16}}>العربية</span> Bahasa Arab
          </div></Rv>
          <Rv delay={0.08}>
            <h1 style={{fontFamily:fh,fontSize:46,fontWeight:800,color:"#fff",lineHeight:1.1,letterSpacing:-1.5,margin:"0 0 10px"}}>
              Baca Quran dengan <span style={{color:"#4ADE9A"}}>Benar</span>.<br/>Paham, Bukan Sekedar Hafal.
            </h1>
            <div style={{fontFamily:fh,fontSize:18,fontWeight:600,color:"rgba(255,255,255,0.4)",marginBottom:20}}>Tajwid, makhraj, dan muhadatsah — dari dasar sampai mahir.</div>
          </Rv>
          <Rv delay={0.12}>
            <p style={{fontFamily:fb,fontSize:16,color:"rgba(255,255,255,0.35)",lineHeight:1.75,margin:"0 0 28px",maxWidth:440}}>
              Kurikulum terstruktur sesuai level. Tutor koreksi makhraj satu per satu — bukan sekedar ikut-ikutan baca. Live via Zoom, untuk anak, remaja, dan Mama.
            </p>
          </Rv>
          <Rv delay={0.16}>
            <a href={WA} target="_blank" rel="noreferrer" style={{fontFamily:fh,fontWeight:700,fontSize:15,color:"#0A1510",background:"#4ADE9A",borderRadius:14,padding:"16px 32px",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 6px 24px rgba(74,222,154,0.25)"}}><WAIcon size={18} color="#0A1510"/> Daftar Kursus Bahasa Arab</a>
          </Rv>
        </div>

        {/* Right — Arabic calligraphy rotating visual */}
        <div className="hero-right" style={{flex:"0 0 360px",position:"relative"}}>
          <Rv delay={0.15}>
            <div style={{background:"rgba(255,255,255,0.03)",borderRadius:24,border:"1px solid rgba(255,255,255,0.06)",padding:"36px 32px",textAlign:"center"}}>
              <div style={{fontFamily:fb,fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",letterSpacing:1,marginBottom:20}}>Yang Anak Anda Pelajari</div>
              <div key={ai} style={{fontSize:52,color:"#4ADE9A",lineHeight:1.2,marginBottom:6,animation:"countUp 0.4s ease",direction:"rtl"}}>{aspects[ai]}</div>
              <div key={`l${ai}`} style={{fontFamily:fh,fontSize:20,fontWeight:700,color:"rgba(255,255,255,0.6)",marginBottom:24,animation:"countUp 0.4s ease"}}>{labels[ai]}</div>

              {/* Curriculum levels */}
              <div style={{display:"flex",gap:6,justifyContent:"center",flexWrap:"wrap"}}>
                {["Pemula","Dasar","Menengah","Mahir"].map((lv,i)=>
                  <div key={lv} style={{fontFamily:fb,fontSize:10,fontWeight:600,color:i<=1?"#4ADE9A":"rgba(255,255,255,0.25)",background:i<=1?"rgba(74,222,154,0.1)":"rgba(255,255,255,0.03)",padding:"5px 12px",borderRadius:6,border:`1px solid ${i<=1?"rgba(74,222,154,0.15)":"rgba(255,255,255,0.04)"}`}}>{lv}</div>
                )}
              </div>
              <div style={{fontFamily:fb,fontSize:10,color:"rgba(255,255,255,0.25)",marginTop:12}}>Kurikulum terstruktur · 4 level · Tajwid + Muhadatsah</div>
            </div>
          </Rv>
          <div style={{position:"absolute",bottom:-8,left:-20,background:"rgba(10,21,16,0.95)",borderRadius:12,padding:"10px 16px",border:"1px solid rgba(74,222,154,0.12)",boxShadow:"0 8px 20px rgba(0,0,0,0.3)",animation:"float 5s ease-in-out infinite",zIndex:2}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:14}}>📖</span>
              <div><div style={{fontFamily:fh,fontSize:11,fontWeight:800,color:"#4ADE9A"}}>Makhraj Benar</div><div style={{fontFamily:fb,fontSize:8,color:"rgba(255,255,255,0.35)"}}>Dikoreksi satu per satu</div></div>
            </div>
          </div>
        </div>
      </div>
...
function AudienceSection(){
  ...
  const audiences = [
    {
      ...
      headline:"Makhraj Benar Sejak Kecil, Tidak Perlu Diperbaiki Nanti",
      desc:"Anak yang belajar makhraj dari kecil punya bacaan yang jauh lebih bersih. Kalau salah dari awal dan dibiarkan, nanti makin susah diperbaiki.",
      pains:["Bacaan Quran anak terbata-bata, makhraj tidak jelas","Di sekolah/TPQ terlalu rame, guru tidak sempat koreksi satu-satu","Anak hafal surat tapi tidak paham tajwid-nya"],
      outcomes:["Makhraj dikoreksi dari awal — kebiasaan yang benar terbentuk","Tajwid dipelajari bertahap sesuai level anak","Anak mulai paham apa yang dibaca, bukan sekedar meniru"],
      story:{name:"Mama Siti",child:"Khalid, 10 thn · Ternate",before:"Baca Quran terbata-bata, makhraj ح dan هـ masih kebalik",after:"Makhraj lebih tepat, bacaan mulai lancar",detail:"Tes Level tunjukkan Khalid sudah hapal beberapa surat pendek tapi makhraj huruf halq masih salah. Tutor fokus koreksi makhraj spesifik — satu huruf satu huruf. Setelah paket pertama, bacaannya jauh lebih bersih.",gender:"boy"},
      waText:"Halo+EduKazia%2C+saya+ingin+daftarkan+anak+untuk+kursus+Bahasa+Arab.+Nama%3A+...+Usia%3A+...",
    },
    {
      label:"Remaja 13–18 thn",icon:"gradcap",c:t.bl,
      headline:"Tajwid yang Benar, Percaya Diri di Pesantren dan Musabaqah",
      desc:"Remaja di pesantren atau madrasah butuh bacaan yang tidak cuma lancar — tapi benar. Di EduKazia, tutor fokus ke koreksi tajwid intensif dan muhadatsah.",
      pains:["Malu sama teman di pesantren karena bacaan masih kurang","Bisa baca tapi tajwid masih banyak salah","Mau ikut musabaqah tapi tidak percaya diri"],
      outcomes:["Tajwid diperbaiki intensif — koreksi spesifik per huruf","Percaya diri meningkat untuk musabaqah dan tilawah","Mulai paham muhadatsah — bukan cuma baca teks"],
      story:{name:"Mama Halimah",child:"Aisyah, 16 thn · Sofifi",before:"Makhraj kurang tepat, malu di pesantren",after:"Dipuji ustadzah, berani ikut musabaqah",detail:"Aisyah bisa baca tapi makhraj huruf tertentu masih salah. Tutor fokus koreksi spesifik + tajwid intensif setiap sesi. Ustadzah di pesantren memuji peningkatannya — sekarang Aisyah lebih percaya diri.",gender:"girl"},
      waText:"Halo+EduKazia%2C+saya+ingin+daftarkan+anak+remaja+untuk+kursus+Bahasa+Arab.+Nama%3A+...+Kelas%3A+...",
    },
    {
      label:"Mama / Dewasa",icon:"users",c:t.p,
      headline:"Bisa Ngajari Anak Ngaji di Rumah — Tanpa Takut Salah",
      desc:"Mama mau bisa ngajari anak ngaji dengan benar. Tidak perlu malu mulai dari nol — tutor sabar, tempo sesuai Mama.",
      pains:["Mau ngajari anak ngaji tapi takut salah tajwid","Bacaan sendiri masih belum yakin benar","Malu belajar Arab di usia segini"],
      outcomes:["Mama bisa ngajari anak ngaji di rumah dengan yakin","Bacaan Quran Mama sendiri membaik — lebih khusyuk","Privat: cuma Mama dan tutor, tidak ada yang tahu"],
      story:{name:"Mama Rahmi",child:"42 thn · Sofifi",before:"Baca Quran terbata-bata, malu sama anak",after:"Bacaan lebih lancar, ngaji bareng anak setelah Maghrib",detail:"Rahmi mau bisa baca Quran benar dan ngajari anak-anaknya. Tutor fokus ke makhraj + tajwid praktis. Sekarang Rahmi ngaji bareng anak setiap habis Maghrib — sesuatu yang dulu tidak pernah dia bayangkan.",gender:"girl"},
      waText:"Halo+EduKazia%2C+saya+Mama+yang+mau+belajar+Bahasa+Arab.+Bisa+konsultasi+dulu%3F",
    },
  ];
  ...
  return <section style={{background:t.bg,padding:"80px 0"}}><W>
    <Rv><div style={{textAlign:"center",marginBottom:44}}>
      <h2 style={{fontFamily:fh,fontSize:38,fontWeight:800,color:t.tx,margin:0,letterSpacing:-0.5,lineHeight:1.15}}>Bahasa Arab untuk Siapa?</h2>
    ...
    {/* Tab selector — Arabic calligraphy-inspired cards */}
    <Rv delay={0.05}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:44}} className="grid-3">
        {audiences.map((au,i)=>{
          const active = tab===i;
          const arabicWords = ["طفل","شباب","أم"];
          const previews = [
            {pain:"Makhraj masih salah",outcome:"Bacaan lebih bersih"},
            {pain:"Tajwid banyak salah",outcome:"Dipuji ustadzah"},
            {pain:"Takut salah ngajari",outcome:"Ngaji bareng anak"},
          ];
          const pv = previews[i];
          return <div key={i} onClick={()=>setTab(i)} style={{
            ...
            <svg style={{position:"absolute",top:0,right:0,opacity:active?0.12:0.06,transition:"opacity 0.4s"}} width="80" height="80" viewBox="0 0 80 80">
              <path d="M0 0h80v80" fill="none" stroke={active?"#fff":au.c} strokeWidth="1"/>
              <path d="M80 0L60 20M80 20L60 40M80 40L60 60M80 60L60 80" fill="none" stroke={active?"#fff":au.c} strokeWidth="0.5"/>
              <circle cx="70" cy="10" r="4" fill="none" stroke={active?"#fff":au.c} strokeWidth="0.5"/>
              <circle cx="70" cy="10" r="2" fill={active?"#fff":au.c} opacity="0.3"/>
            </svg>
            <svg style={{position:"absolute",bottom:0,left:0,opacity:active?0.1:0.04,transition:"opacity 0.4s",transform:"rotate(180deg)"}} width="60" height="60" viewBox="0 0 80 80">
              <path d="M0 0h80v80" fill="none" stroke={active?"#fff":au.c} strokeWidth="1"/>
              <path d="M80 0L60 20M80 20L60 40M80 40L60 60" fill="none" stroke={active?"#fff":au.c} strokeWidth="0.5"/>
            </svg>

            <div style={{position:"relative",padding:"24px 22px 22px"}}>
              {/* Arabic watermark + label row */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                <div>
                  <div style={{fontSize:36,lineHeight:1,color:active?"rgba(255,255,255,0.15)":`${au.c}12`,direction:"rtl",fontWeight:700,marginBottom:4,transition:"color 0.3s"}}>{arabicWords[i]}</div>
                  <h3 style={{fontFamily:fh,fontSize:20,fontWeight:800,color:active?"#fff":t.tx,margin:0,lineHeight:1.2,transition:"color 0.3s"}}>{au.label}</h3>
                </div>
                {active&&<div style={{width:24,height:24,borderRadius:"50%",background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 5l2.5 2.5 4-4" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></div>}
              </div>

              {/* Divider — geometric line */}
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
                <div style={{flex:1,height:1,background:active?"rgba(255,255,255,0.15)":`${au.c}15`}}/>
                <div style={{width:6,height:6,transform:"rotate(45deg)",background:active?"rgba(255,255,255,0.2)":`${au.c}20`}}/>
                <div style={{flex:1,height:1,background:active?"rgba(255,255,255,0.15)":`${au.c}15`}}/>
              </div>

              {/* Before → After as "scroll" style */}
              <div style={{marginBottom:14}}>
                <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
                  <div style={{width:14,height:14,borderRadius:4,background:active?"rgba(255,255,255,0.1)":`${t.coral}08`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:active?"rgba(255,255,255,0.5)":t.coral}}>✗</div>
                  <span style={{fontFamily:fb,fontSize:12,fontWeight:600,color:active?"rgba(255,255,255,0.6)":t.coral}}>{pv.pain}</span>
                </div>
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  <div style={{width:14,height:14,borderRadius:4,background:active?"rgba(255,255,255,0.15)":`${au.c}10`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:active?"#fff":au.c}}>✓</div>
                  <span style={{fontFamily:fb,fontSize:12,fontWeight:600,color:active?"#fff":au.c}}>{pv.outcome}</span>
                </div>
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                ...
                <div style={{width:22,height:22,borderRadius:4,transform:"rotate(45deg)",background:active?"rgba(255,255,255,0.15)":`${au.c}08`,display:"flex",alignItems:"center",justifyContent:"center",border:active?"1px solid rgba(255,255,255,0.15)":`1px solid ${au.c}12`}}>
                  <svg style={{transform:"rotate(-45deg)"}} width="8" height="8" viewBox="0 0 8 8" fill="none"><path d={active?"M1.5 4l1.5 1.5 3-3":"M2.5 1.5L5 4L2.5 6.5"} stroke={active?"#fff":au.c} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
              </div>
            </div>
          </div>;
    ...
    <div key={tab} style={{animation:"countUp 0.4s ease"}}>
      ...
      {false && (
        ...
        <div style={{background:a.c,padding:"14px 28px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontFamily:fh,fontSize:13,fontWeight:700,color:"#fff"}}>{tab===2?a.story.name:`${a.story.name} — ${a.story.child}`}</div><div style={{fontFamily:fb,fontSize:10,color:"rgba(255,255,255,0.65)"}}>Kursus Bahasa Arab</div></div>
        </div>
...
function ClassTypes(){
  const t=useT();
  const classes=[
    {name:"Reguler",ratio:"1 : 8",desc:"Belajar bareng teman. Kompetisi sehat, biaya terjangkau.",c:t.v,tags:["Biaya ringan","Ada teman","Seru"]},
    {name:"Semi Privat",ratio:"1 : 4",desc:"Kelompok kecil — tutor lebih kenal karakter siswa.",c:t.or,tags:["Perhatian lebih","Kelompok nyaman"]},
    {name:"Privat",ratio:"1 : 1",desc:"Perhatian 100%. Makhraj dikoreksi satu per satu. Jadwal fleksibel.",c:t.p,rec:true,tags:["Perhatian penuh","Koreksi individual","Reschedule gratis"]},
  ];
  return <section style={{background:t.bgW,padding:"80px 0"}}><W>
    <Rv><div style={{textAlign:"center",marginBottom:44}}>
      ...
      <p style={{fontFamily:fb,fontSize:15,color:t.sub,marginTop:10,lineHeight:1.7}}>Kurikulum berbasis level untuk semua tipe kelas.</p>
    </div></Rv>
...
function TesLevelAndFAQ(){
  ...
  const faqs=[
    ["Anak belum bisa baca Quran sama sekali, bisa?","Bisa. Kami punya kurikulum dari level paling dasar — huruf hijaiyah, harakat, sampai tajwid."],
    ["Fokusnya ke Quran atau muhadatsah?","Dua-duanya tersedia. Tes Level tentukan kebutuhan siswa, lalu tutor fokuskan sesuai prioritas."],
    ["Mama juga bisa ikut?","Bisa. Banyak Mama yang belajar — supaya bisa ngajari anak ngaji di rumah."],
    ["Jadwalnya bagaimana?","Privat 100% fleksibel. Reguler & Semi Privat ada slot pilihan."],
    ["Ada rekaman kelas?","Semua sesi direkam. Siswa bisa tonton ulang."],
    ["Pembayarannya bagaimana?","Per paket — ringan. Detail harga via WhatsApp."],
  ];
  return <section id="tes-level" style={{background:t.bg,padding:"80px 0"}}><W>
    <div className="story-flex" style={{display:"flex",gap:40}}>
      <div style={{flex:"1 1 360px"}}>
        ...
        <p style={{fontFamily:fb,fontSize:14,color:t.sub,lineHeight:1.7,marginBottom:28}}>Supaya tutor tahu level bacaan dan makhraj sekarang — dan tahu dari mana harus mulai.</p></Rv>
        <Rv delay={0.05}><div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[{n:"1",icon:<MessageSquare size={22} color={t.p}/>,title:"Chat WhatsApp",desc:"Bilang siapa yang mau belajar dan usia berapa."},{n:"2",icon:<ClipboardCheck size={22} color={t.p}/>,title:"Tes Level Online",desc:"30 menit via Zoom. Baca beberapa ayat, tutor evaluasi."},{n:"3",icon:<BarChart3 size={22} color={t.p}/>,title:"Terima Laporan",desc:"Level bacaan, makhraj yang perlu diperbaiki, rekomendasi."},{n:"4",icon:<Zap size={22} color={t.p}/>,title:"Mulai Belajar",desc:"Pilih tipe kelas, mulai sesuai level."}].map((s,i)=>
            <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0}}>
                <div style={{width:34,height:34,borderRadius:10,background:t.pS,display:"flex",alignItems:"center",justifyContent:"center"}}>{ s.icon==="gradcap"?<GraduationCap size={16} color="currentColor"/>:<Users size={16} color="currentColor"/>}</div>
        ...
        <Rv delay={0.1}><div style={{marginTop:24}}>
          <a href={`https://wa.me/6281384253679?text=Halo+EduKazia%2C+saya+ingin+Tes+Level+Bahasa+Arab`} target="_blank" rel="noreferrer" style={{fontFamily:fh,fontWeight:700,fontSize:13,color:t.goldT,background:t.gold,borderRadius:12,padding:"13px 28px",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:6,boxShadow:"0 4px 16px rgba(230,184,0,0.2)"}}><WAIcon size={14} color="#1A1535"/> Daftar Tes Level</a>
        </div></Rv>
      </div>
      <div style={{flex:"1 1 380px"}}>
        <Rv><h3 style={{fontFamily:fh,fontSize:20,fontWeight:800,color:t.tx,margin:"0 0 18px"}}>Pertanyaan tentang Bahasa Arab</h3></Rv>
        <Rv delay={0.05}><div>
...
function OtherSubjects(){
  const t=useT();
  const others=[
    {name:"Bahasa Inggris",desc:"Speaking, conversation, pronunciation. Percaya diri ngomong.",href:"/kursus/bahasa-inggris",c:t.bl},
  ...
  return <section style={{background:t.bgW,padding:"64px 0"}}><W>
    <Rv><h3 style={{fontFamily:fh,fontSize:24,fontWeight:800,color:t.tx,margin:"0 0 24px"}}>Pelajaran Lainnya di EduKazia</h3></Rv>
    <div className="grid-3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
      {others.map((s,i)=>{const[h,setH]=useState(false);return <Rv key={i} delay={0.04*(i+1)}>
        <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{background:t.card,borderRadius:14,padding:"20px",border:`1.5px solid ${h?s.c+"25":t.cb}`,boxShadow:h?t.ch:t.cs,transition:"all 0.3s",display:"flex",gap:12,alignItems:"flex-start"}}>
          <div style={{width:4,height:28,borderRadius:2,background:s.c,flexShrink:0,marginTop:3}}/>
          <div><h4 style={{fontFamily:fh,fontSize:15,fontWeight:700,color:t.tx,margin:"0 0 4px"}}>{s.name}</h4><p style={{fontFamily:fb,fontSize:12,color:t.sub,lineHeight:1.5,margin:"0 0 10px"}}>{s.desc}</p><a href={s.href} style={{fontFamily:fh,fontWeight:700,fontSize:11,color:s.c,textDecoration:"none"}}>Lihat →</a></div>
        </div>
      </Rv>;})}
    </div>
  </W></section>;
...
function CTAFinal(){
  ...
  return <section style={{background:t.bg,padding:"80px 0"}}><W><Rv>
    <div style={{background:"linear-gradient(135deg,#0A1510,#122820)",borderRadius:28,overflow:"hidden",position:"relative"}}>
      <div style={{position:"absolute",top:-30,right:-30,width:150,height:150,borderRadius:"50%",background:"#10B981",opacity:0.06,filter:"blur(50px)"}}/>
      <div style={{padding:"56px 48px",textAlign:"center",position:"relative",zIndex:1}}>
        <div style={{fontSize:36,marginBottom:10,direction:"rtl",color:"#4ADE9A",fontWeight:700}}>العربية</div>
        <h2 style={{fontFamily:fh,fontSize:30,fontWeight:800,color:"#fff",margin:"0 0 12px",lineHeight:1.2}}>Bacaan yang Benar<br/>Dimulai Sekarang.</h2>
        <p style={{fontFamily:fb,fontSize:15,color:"rgba(255,255,255,0.4)",lineHeight:1.7,marginBottom:28,maxWidth:440,marginLeft:"auto",marginRight:"auto"}}>Chat langsung. Kami bantu tentukan level, tipe kelas, dan jadwal yang pas.</p>
        <a href={WA} target="_blank" rel="noreferrer" style={{fontFamily:fh,fontWeight:700,fontSize:16,color:"#0A1510",background:"#4ADE9A",borderRadius:14,padding:"16px 40px",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 6px 24px rgba(74,222,154,0.25)"}}><WAIcon size={18} color="#0A1510"/> Daftar Kursus Bahasa Arab</a>
...
export default function KursusBahasaArab(){
